...
public class RequirementStatusService {
        ...
        public List<RequirementStatus> getStatuses() {
                ...
                List<RequirementStatus> requirementStatusList = requirementStatusRepository
        ...
        public String upsertStatus(int requirementId, int status) {
                ...
                RequirementStatus requirementStatus = requirementStatusRepository
                                .findByRequirementIdAndUserId(requirementId, currentUser.getId())
                                .orElseGet(() ->
        ...
        public int getStatusProgress() {
                ...
                List<RequirementStatus> existingStatuses = requirementStatusRepository
                                .findByUserId(currentUser.getId());

                // Fetch all requirements
                List<Requirement> allRequirements = requirementRepository.findAll();

                // Build a complete list: existing statuses + in-memory defaults for missing
                // requirements
        ...
        public List<RequirementStatus> getStatusesByCompanyId(int companyId) {
                // Get all internal auditors for this company
                // RequirementStatus is linked to users, and users are linked to companies
                // We need to find all requirement statuses for users (internal auditors) in
                // this company
                List<RequirementStatus> allStatuses = requirementStatusRepository.findAll();

                // Filter statuses by users who belong to the specified company
