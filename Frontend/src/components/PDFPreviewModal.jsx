...
export default function PDFPreviewModal({ document, isOpen, onClose, pdfUrl: directPdfUrl, fileName: directFileName }) {
  if (!isOpen) return null

  // Support both document object and direct URL
  let pdfUrl = directPdfUrl
  let displayFileName = directFileName || "PDF Preview"

  if (document && !directPdfUrl) {
    const isPDF = document.fileType === "application/pdf" || document.fileName?.toLowerCase().endsWith(".pdf")
    displayFileName = document.fileName || "PDF Preview"
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-2 z-50 animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl max-w-7xl w-full h-[95vh] overflow-hidden flex flex-col animate-scale-in">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-gradient-to-r from-indigo-50 to-purple-50">
          <h3 className="text-lg font-bold text-gray-900">{displayFileName}</h3>
          <button
            onClick={onClose}
